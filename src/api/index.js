...
import {authRoute, crudRoute, databaseRoute, listenRoute} from './api-route.js'
...
class SwygerClient{
  ...
  constructor(config={}) {
    ...
    this.#db=AceBase.WithIndexedDB(OFFLINE_DB_NAME?.DATABASE||'swyger_database') //db = localStorage
  ...
  init=(req)=>{
    ...
    req?.socket?.database?.on('connect',()=>{
      //console.log('device connected')
      this.#dataBaseConnected=true
      offlineDatabaseHandler()
    })
    req?.socket?.database?.on('disconnect',()=>{
    ...
    const listenEmitter=(ref,callback=()=>{})=>{
      ...
      let eventName=API_VERSION?.DATABASE+listenLocal+ref
      eventName=eventName.replace(/\/\//g,'/')
    ...
    let localDatabase=(method,path,data={},node,callback)=>{
      ...
      let handlerRequest=async ()=>{
        ...
        const listenLocal=this.#listen
        let eventName=API_VERSION?.DATABASE+listenLocal+path+'/'+method
    ...
    return {
      ...
      database:()=>{
        const database=this.init(req)
        delete database.database
        delete database.auth().register
        delete database.auth().login
        req?.socket?.database.on('connect',()=>{
          this.#dataBaseConnected=true
          offlineDatabaseHandler()
        })
        ...
        return apiService(database,{
          host:HOST_SERVER.DATABASE,
          version:API_VERSION.DATABASE,
          base:databaseRoute.base
        },req?.socket?.database)
      },
      cron:(config)=>{},
      task:(config)=>{},
        ...
        return {
          ...
          create: (...args) => {
            ...
            if (parent.auth().$currentUserId && config?.host === HOST_SERVER?.DATABASE && this.#syncWithLocal && !this.#dataBaseConnected) {
              ...
              return parent.local()?.database()?.ref(path)?.create(...args)
            } else {
              //do online request
          ...
          find: (...args) => {
            console.log('Database connected: ', this.#dataBaseConnected)
            if (parent.auth().$currentUserId && config?.host === HOST_SERVER?.DATABASE && this.#syncWithLocal && !this.#dataBaseConnected) {
              //save data when offline to transaction and database locally, then do background process
              // when online to send data and erase previous transaction
          ...
          get: (...args) => {
            if (parent.auth().$currentUserId && config?.host === HOST_SERVER?.DATABASE && this.#syncWithLocal && !this.#dataBaseConnected) {
              //save data when offline to transaction and database locally, then do background process
              // when online to send data and erase previous transaction
              return parent.local()?.database()?.ref(path)?.get(...args)
            } else {
              //do online request
              let node = {
                ...parent.ref(path, config, socket),
          ...
          update: (...args) => {
            if (parent.auth().$currentUserId && config?.host === HOST_SERVER?.DATABASE && this.#syncWithLocal && !this.#dataBaseConnected) {
              //save data when offline to transaction and database locally, then do background process
              // when online to send data and erase previous transaction
              return parent.local()?.database()?.ref(path)?.update(...args)
            } else {
              //do online request
              let node = {
                ...parent.ref(path, config, socket),
